...
class AvatarView extends StatelessWidget {
  ...
  Widget _normalAvatar() {
    ...
    // 检查是否是相对路径URL（不包含协议但有内容）
    bool isRelativePath = !url!.contains('://') && !url!.contains('file:') && url!.isNotEmpty;
    if (isRelativePath) {
      // 对于相对路径，直接使用convertMediaUrl进行转换
      if (convertUrl) {
        final fullUrl = UrlConverter.convertMediaUrl(url!);
        ...
        return ImageUtil.networkImage(
          ...
          convertUrl: false, // 已经手动转换，不需要再次转换
        );
      }
    }
    ...
    if (isGroup && url != null && url!.isNotEmpty) {
      // 尝试转换群组头像URL
      final convertedUrl = UrlConverter.convertGroupAvatarUrl(url!);

      // 如果URL转换后发生变化，则使用转换后的URL
      if (convertedUrl != url) {
        return ImageUtil.networkImage(
          url: convertedUrl,
          width: _avatarSize,
  ...
  Widget _networkImageAvatar() => file != null
      ...
      : ImageUtil.networkImage(
          ...
          loadProgress: false,
          errorWidget: _textAvatar(),
          convertUrl: convertUrl, // 使用传入的convertUrl参数
        );
