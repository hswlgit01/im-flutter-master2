...
class InputBox extends StatefulWidget {
  ...
  const InputBox.verificationCode({
    ...
    this.keyBoardType,
        ...
        clearBtnColor = null,
        onImageVerificationTap = null;
  const InputBox.imageVerificationCode({
    super.key,
    required this.label,
    this.controller,
    this.focusNode,
    ...
    this.onImageVerificationTap,
  })  : obscureText = false,
        type = InputBoxType.imageVerificationCode,
        code = '',
        codeStyle = null,
        onAreaCode = null,
        arrowColor = null,
  ...
  final VoidCallback? onImageVerificationTap;
  final EdgeInsetsGeometry? margin;
  final List<TextInputFormatter>? inputFormatters;
  final TextInputType? keyBoardType;
  final Widget? verificationImage;
          ...
          Container(
            ...
            child: Row(
              ...
              children: [
                ...
                if (widget.type == InputBoxType.imageVerificationCode)
                  ImageVerification(
                    verificationImage: widget.verificationImage,
                    onTap: widget.onImageVerificationTap,
                  )
              ],
            ),
          ),          if (widget.type == InputBoxType.imageVerificationCode) Padding(
              padding: EdgeInsets.only(top: 5.h),
              child: Align(
                alignment: Alignment.centerRight,
                child: GestureDetector(
                  onTap: widget.onImageVerificationTap,
                  child: StrRes.captchaRefreshPrompt.toText
                  ..style = (widget.formatHintStyle ?? Styles.ts_0089FF_12sp),
                ),
              ),
            ),
  ...
  TextInputType? get _textInputType {
    ...
    switch (widget.type) {
      ...
      case InputBoxType.invitationCode:
        keyboardType = TextInputType.text;
        break;
      case InputBoxType.imageVerificationCode:
        keyboardType = TextInputType.text;
        break;
    }
    return keyboardType;
...
class ImageVerification extends StatelessWidget {
  ...
  const ImageVerification({super.key, this.verificationImage, this.onTap});
  @override
  Widget build(BuildContext context) {
    // Placeholder for image verification widget
