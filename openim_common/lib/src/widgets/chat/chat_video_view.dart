...
class _ChatVideoViewState extends State<ChatVideoView> {
  ...
  void initState() {
    ...
    _sourcePath = video?.snapshotPath;

    _sourceUrl = video?.snapshotUrl;
  ...
  Widget _buildUrlPicture({required String url}) => ImageUtil.networkImage(
        url: url, // 现在通过 ImageUtil.networkImage 自动转换
        height: _trulyHeight,
        width: _trulyWidth,
        fit: BoxFit.fitWidth,
      );
  ...
  Widget build(BuildContext context) {
    final child = ClipRRect(
      borderRadius: widget.radius ? const BorderRadius.all(Radius.circular(6.0)) : borderRadius(widget.isISend),
      ...
      children: [
        ...
        Positioned.fill(
          ...
          child: LayoutBuilder(builder: (_, constraints) {
            return ImageRes.videoPause.toImage..width = constraints.maxWidth * 0.4;
          }),
        ))
      ],
    );
