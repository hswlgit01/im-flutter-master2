...
class ChatLogic extends SuperController with WidgetsBindingObserver {
  ...
  initMessageList() async {
    ...
    enabledTopLoad.value = await onScrollToTopLoad();
    _applyPendingRevokeDetails();
  ...
  Future<void> _reconcileSendStatus(Message message, bool useOuterValue) async {
    ...
    try {
      ...
      if (stored != null && stored.status == MessageStatus.failed) {
        ...
        sendStatusSub
            .addSafely(MsgStreamEv<bool>(id: clientMsgID, value: false));
      } else {
      ...
      app_log.LogUtil.e(
          'ChatLogic', '发送超时对账查询失败,乐观清圈: clientMsgID=$clientMsgID', e);
      _markSendStatusOnList(clientMsgID, MessageStatus.succeeded);
    }
  }
  ...
  /// 确保留在页面时转圈也立即消失。
  void _markSendStatusOnList(String clientMsgID, int status,
      {Message? stored}) {
    var hit = false;
