...
class MyTeamLogic extends GetxController {
  ...
  final RxBool isDownloadUrlLoading = false.obs;
  ...
  Future<void> initializeData() async {
    ...
    try {
      ...
      // 获取团队信息
      await refreshUserInfo(updateLoading: false);

      // 获取下载链接
      await getDownloadUrl();
    } catch (e) {
      LogUtil.e(_tag, '初始化数据失败: $e');
    } finally {
  ...
  Future<void> refreshRemoteConfig() async {
    try {
      ...
      LogUtil.i(_tag, '远程配置刷新完成');
    } catch (e) {
      LogUtil.e(_tag, '刷新远程配置失败: $e');
    }
  }

  // 刷新用户信息（包含团队数据）
  Future<void> refreshUserInfo({bool updateLoading = true}) async {
    try {
      if (updateLoading) {
        isLoading.value = true;
      }
      ...
      if (teamInfoData != null) {
        ...
        LogUtil.i(_tag,
            '从团队API获取的数据: teamSize=${teamInfoData.teamSize}, directDownlineCount=${teamInfoData.directDownlineCount}, invitationCode=${teamInfoData.invitationCode}');
      } else {
        // 如果获取失败，使用空数据
  ...
  Future<void> getDownloadUrl() async {
    isDownloadUrlLoading.value = true;
    try {
      LogUtil.i(_tag, '开始获取应用下载链接...');
      ...
      if (remoteConfig != null &&
        ...
        LogUtil.i(_tag, '成功获取app_version配置: ${appVersionConfig.keys}');
        ...
        if (Platform.isAndroid && appVersionConfig.containsKey('android')) {
          ...
          if (downloadLink != null && downloadLink.isNotEmpty) {
            ...
            LogUtil.i(_tag, '从远程配置获取的Android下载链接: $downloadLink');
            return;
          }
          ...
          if (downloadLink != null && downloadLink.isNotEmpty) {
            downloadUrl.value = downloadLink;
            LogUtil.i(_tag, '从远程配置获取的iOS下载链接: $downloadLink');
            return;
          }
        }
      }
      ...
      if (fallbackLink != null) {
        ...
        LogUtil.i(_tag, '从远程配置获取的通用下载链接: $fallbackLink');
        return;
      }
      ...
      // 异常情况下链接保持为空
      LogUtil.e(_tag, '获取下载链接异常: $e，链接为空');
    } finally {
      isDownloadUrlLoading.value = false;
    }
  }
