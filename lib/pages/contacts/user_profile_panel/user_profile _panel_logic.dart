...
class UserProfilePanelLogic extends GetxController {
  ...
  muteSetup() {
    AppNavigator.startMuteSetup(
        groupID: groupID, userID: userInfo.value.userID!);
  }
  ...
  void setAdminPermission(bool isAdmin) async {
    if (groupID == null || userInfo.value.userID == null) return;
    try {
      ...
      await OpenIM.iMManager.groupManager.setGroupMemberInfo(
        groupMembersInfo: SetGroupMemberInfo(
            groupID: groupID!,
            userID: userInfo.value.userID!,
            roleLevel: isAdmin ? GroupRoleLevel.admin : GroupRoleLevel.member),
      );
      hasAdminPermission.value = isAdmin;
      await EasyLoading.dismiss();
      IMViews.showToast(isAdmin ? '设置为管理员成功' : '取消管理员成功');
    } catch (e) {
